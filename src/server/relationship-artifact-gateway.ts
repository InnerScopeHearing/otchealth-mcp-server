...
function defaultDeps(injected?:Partial<RelationshipArtifactGatewayDeps>):RelationshipArtifactGatewayDeps {
  ...
  return {authenticate:injected?.authenticate??requireConnectorAuth,policyJson:injected?.policyJson??(()=>loadEnv().GRAPH_RELATIONSHIP_ARTIFACT_POLICY_JSON),now:injected?.now??Date.now,s3,resolveCohortBinding:injected?.resolveCohortBinding??(async(ctx,run,signal)=>{const catalog=await import('./graph-catalog-controller.js');return catalog.resolveCatalogCohortBinding(ctx,run,signal,{now:injected?.now??Date.now,configs:()=>loadEnv().GRAPH_CATALOG_COHORTS_JSON});}),resolveAutomaticBinding:injected?.resolveAutomaticBinding??(async(ctx,run,producer,signal)=>{const publication=await import('./relationship-publication.js');return publication.resolveRelationshipArtifactAutomaticBinding({ctx,run_id:run,producer_id:producer,signal},{now:injected?.now??Date.now});})};
}
