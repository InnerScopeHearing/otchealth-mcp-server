...
import {createCompanyTextSnapshotReader} from '../graph/company-text-snapshot.js';
...
function validRun(r:any){if(!exact(r,['ref_version','run_id','purpose','scope','run_version','manifest_sha256']))return false;const{run_id,...body}=r;return r.ref_version==='neptune-trial-active-run-ref-v1'&&['finance','legal_company'].includes(r.scope)&&LABEL.test(r.purpose)&&LABEL.test(r.run_version)&&SHA.test(r.manifest_sha256)&&run_id==='run_'+hash(canonical(body));}
function scopeFor(binding:any){const scope=resolveCompanyGraphScope(binding?.authenticated_caller,binding?.run?.scope);return scope.ok?scope.scope:null;}
...
function parse(text:string,now:number):Json|null{
 ...
 const seen=new Set();for(const b of p.bindings){
  if(!exact(b,['authenticated_caller','caller_hash','producer_id','run','encryption','cohort_id','admission','proposal','approved_artifacts','source_policy'])||!scopeFor(b)||!SHA.test(b.caller_hash)||!PRODUCER.test(b.producer_id)||!validRun(b.run)||!LABEL.test(b.cohort_id)||b.cohort_id==='.'||b.cohort_id==='..'||!validRef(b.admission)||!validRef(b.proposal))return null;
...
function parseArtifact(r:HistoricalReadResult,b:Json,digest:string,v:string):Json{
 ...
 const p=e.payload;if(p.schema==='resolution-source-input-v1'){if(!exact(p,['schema','run','input']))throw Error('artifact');}else if(p.schema!=='resolution-history-v1'||!exact(p,['schema','run','caller_seat','sources','events','queries'])||p.caller_seat!==b.authenticated_caller||!Array.isArray(p.sources)||!p.sources.length||p.sources.length>64||!p.sources.every(artifactRef)||new Set(p.sources.map(canonical)).size!==p.sources.length||!Array.isArray(p.events)||!Array.isArray(p.queries))throw Error('artifact');return e;
}
function sourceBound(source:Json,b:Json,proposal:Json){
 ...
 const scope=scopeFor(b),x=i.binding;if(!scope||!exact(x,['schema','run_id','room','source_index','catalog_manifest_sha256','document_ordinal','source_document_version','catalog_source_sha256','snapshot_id','prepared_manifest_sha256','sidecar_content_sha256','chunk_ordinal','chunk_sha256'])||!['cfo-prepared-chunk-binding-v1','company-prepared-chunk-binding-v1'].includes(x.schema)||x.run_id!==b.run.run_id||x.room!==scope.room||x.source_index!==scope.sourceIndex||x.catalog_manifest_sha256!==b.run.manifest_sha256||x.document_ordinal!==0||!/^txtsnap_[a-f0-9]{64}$/.test(x.snapshot_id)||!SHA.test(x.prepared_manifest_sha256)||!SHA.test(x.sidecar_content_sha256)||!SHA.test(x.chunk_sha256)||!Number.isSafeInteger(x.chunk_ordinal)||x.chunk_ordinal<0)return false;
