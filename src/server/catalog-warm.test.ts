...
// the full tool surface (>= the pipeline's MIN_TOOLS floor of 800).
before(() => {
...
test('registerAllTools warms the capability catalog to the full tool surface (deploy gate guard)', async () => {
  ...
  const n = toolCount();
  assert.ok(n >= 800, `expected the warmed catalog to expose >= 800 tools (the deploy MIN_TOOLS floor), got ${n}`);
});
