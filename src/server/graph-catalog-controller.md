...
The catalog reader sends HEAD then GET with If-Match, checks exact ETag and byte length, derives its timestamp from Last-Modified, and bounds JSONL to 192 MiB, 100,000 rows and 1 MiB per line. Selected metadata remains in the CFO boundary. The gateway does not return full catalog contents to the host. Each page, publication and current-source check performs a fresh HEAD, including the exact ETag, Last-Modified, size and version ID used in the cache identity. An unchanged catalog within the bounded cache limits reuses its verified parsed rows after that HEAD, so the 24 MiB and 50,000 row materialized-catalog case avoids repeating GET and JSONL parsing. A changed or revoked object fails the fresh HEAD or produces a new identity and cannot reuse the prior rows.
