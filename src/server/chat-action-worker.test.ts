...
test('worker claims, dispatches, and persists terminal success', async () => {
  ...
  const job: any = { id: 'caj_' + 'a'.repeat(64), type: 'chat_action_job', caller_hash: 'caller-a-123456', caller_agent: 'coo', action: 'brain_search', request: { query: 'synthetic' }, status: 'queued', created_at: new Date().toISOString(), updated_at: new Date().toISOString(), idempotency_key_hash: 'x' };
  const writes: any[] = []; let dispatched = false;
  const deps: any = { readDoc: async () => job, replaceDoc: async (_c: string, _p: string, _id: string, doc: any) => { writes.push(doc); Object.assign(job, doc); }, queryDocs: async () => [job], execute: { brainSearch: async (request: any, context: any) => { dispatched = request.query === 'synthetic' && context.callerHash === job.caller_hash && context.callerAgent === 'coo'; return { count: 0 }; }, checkpoint: async () => ({}) } };
...
test('worker rejects protected personal content before claiming', async () => {
  const { runQueuedChatActionJob } = await import('./chat-action-worker.js');
  const job: any = { id: 'caj_' + 'b'.repeat(64), type: 'chat_action_job', caller_hash: 'caller-a-123456', caller_agent: 'clo-personal', action: 'checkpoint', request: { agent: 'clo-personal' }, status: 'queued', created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
  let writes = 0;
