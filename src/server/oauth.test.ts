...
test('E2E: a DCR client redeeming a WeFunder setup code reaches only the dedicated principal across refresh', async () => {
  const { default: Fastify } = await import('fastify');
  const { registerOAuthRoutes, issuedAgent } = await import('./oauth.js');
  const { mintSetupCode } = await import('../auth/setup-codes.js');
  ...
  const minted = await mintSetupCode({ role: 'wefunder-campaign-director', createdBy: 'cto' }, setupCode);
  const clientId = await registerDcrClient(app);
  const { verifier, challenge } = pkcePair();
  const authRes = await app.inject({
    method: 'GET',
  ...
  assert.equal(tokenRes.statusCode, 200, JSON.stringify(tokenRes.json()));
  assert.equal(issuedAgent(tokenRes.json().access_token), 'wefunder-campaign-director');
  assert.notEqual(issuedAgent(tokenRes.json().access_token), 'cro');

  const refreshRes = await app.inject({
    method: 'POST',
    url: '/oauth/token',
    headers: { 'content-type': 'application/x-www-form-urlencoded' },
    payload: `grant_type=refresh_token&refresh_token=${tokenRes.json().refresh_token}`,
  });
  assert.equal(refreshRes.statusCode, 200);
  assert.equal(issuedAgent(refreshRes.json().access_token), 'wefunder-campaign-director');
  assert.notEqual(issuedAgent(refreshRes.json().access_token), 'cro');
  await app.close();
});
...
test('E2E: a chat_shared setup code issues the fixed shared principal and preserves it on refresh', async () => {
  const { default: Fastify } = await import('fastify');
  const { registerOAuthRoutes, issuedAgent } = await import('./oauth.js');
  const { mintSetupCode } = await import('../auth/setup-codes.js');
  const { validateBearer } = await import('../auth/bearer.js');
  ...
  const minted = await mintSetupCode({ role: 'chat_shared', createdBy: 'cto' }, setupCode);
  const clientId = await registerDcrClient(app);
  const { verifier, challenge } = pkcePair();
  const authRes = await app.inject({
    method: 'GET',
  ...
  assert.equal(tokenRes.statusCode, 200, JSON.stringify(tokenRes.json()));
  const body = tokenRes.json();
  assert.equal(issuedAgent(body.access_token), 'chat_shared');
  const bearerContext = await validateBearer(`Bearer ${body.access_token}`);
  assert.equal(bearerContext?.caller_agent, 'chat_shared');
  assert.equal(bearerContext?.connector_surface, true, 'the elevated shared principal must retain connector-surface curation');
  ...
  assert.equal(refreshRes.statusCode, 200);
  assert.equal(issuedAgent(refreshRes.json().access_token), 'chat_shared');
  await app.close();
});
