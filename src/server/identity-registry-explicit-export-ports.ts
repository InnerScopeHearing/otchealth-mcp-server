...
// An alias can be repointed. Pin the immutable key ARN in public configuration.
const KMS_KEY = /^arn:aws:kms:us-east-1:900915535335:key\/[a-f0-9-]{36}$/;
...
async function boundedJson(response: Response): Promise<Record<string, unknown>> {
  ...
  const reader = response.body.getReader(), chunks: Buffer[] = []; let size = 0, complete = false;
  try {
    for (;;) { const part = await reader.read(); if (part.done) { complete = true; break; } size += part.value.byteLength; if (size > MAX_RESPONSE) fail('identity_export_kms_unavailable'); chunks.push(Buffer.from(part.value)); }
...
export async function createCfoIdentityRegistryKmsSigner(config: CfoIdentityRegistryKmsSignerConfig): Promise<Readonly<{ publicKey: string; maxMessageBytes: 4096; sign(bytes: Buffer): Promise<Buffer> }>> {
  ...
  if (publicResponse.KeyId !== config.keyId || publicResponse.KeySpec !== 'ECC_NIST_EDWARDS25519' || publicResponse.KeyUsage !== 'SIGN_VERIFY' ||
      !Array.isArray(publicResponse.SigningAlgorithms) || !publicResponse.SigningAlgorithms.includes('ED25519_SHA_512') ||
      typeof publicResponse.PublicKey !== 'string' || !/^[A-Za-z0-9+/]+={0,2}$/.test(publicResponse.PublicKey)) fail('identity_export_kms_unavailable');
  let key;
  ...
  return Object.freeze({ publicKey, maxMessageBytes: 4096 as const, async sign(bytes: Buffer): Promise<Buffer> {
    if (!Buffer.isBuffer(bytes) || bytes.length > 4096) fail('identity_export_signature_invalid');
    const signed = await call('TrentService.Sign', { KeyId: config.keyId, Message: bytes.toString('base64'), MessageType: 'RAW', SigningAlgorithm: 'ED25519_SHA_512' });
    if (signed.KeyId !== config.keyId || signed.SigningAlgorithm !== 'ED25519_SHA_512' || typeof signed.Signature !== 'string' || !/^[A-Za-z0-9+/]+={0,2}$/.test(signed.Signature)) fail('identity_export_signature_invalid');
...
export function createExplicitExportImmutableStore(config: ExplicitExportImmutableStoreConfig): Readonly<{ maxVersionIdBytes: 128; putImmutable(input: { key: string; body: Buffer }): Promise<{ version_id: string }> }> {
  ...
  return Object.freeze({ maxVersionIdBytes: 128 as const, async putImmutable({ key, body }) {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        const version_id = version(response.headers); if (version_id.length > 128) fail('identity_export_store_invalid');
      ...
      if (code === 'identity_export_store_conflict' || code === 'identity_export_store_invalid') throw error;
    }
    const existing = await read(key, body, signal);
    if (!existing) fail('identity_export_store_invalid');
    if (existing.version_id.length > 128) fail('identity_export_store_invalid');
    return existing;
  } });
}
