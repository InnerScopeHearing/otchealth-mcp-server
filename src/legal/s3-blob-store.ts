...
export class S3WriteTransportError extends Error {
  readonly container: string;
  readonly path: string;

  constructor(container: string, path: string, cause: unknown) {
    super('s3 blob put transport outcome is unknown', { cause });
    this.name = 'S3WriteTransportError';
    this.container = container;
    this.path = path;
  }
}

export class S3WriteHttpError extends Error {
  readonly status: number;
  readonly container: string;
  readonly path: string;

  constructor(status: number, container: string, path: string, detail: string) {
    super(`s3 blob put ${status}: ${detail}`);
    this.name = 'S3WriteHttpError';
    this.status = status;
    this.container = container;
    this.path = path;
  }
...
export async function putObjectToS3(
  ...
  try {
    ...
    throw new S3WriteTransportError(container, path, error);
  }
  if (r.status === 409 || r.status === 412) {
    throw new S3ObjectAlreadyExistsError(r.status, container, path);
  }
  if (!r.ok) throw new S3WriteHttpError(r.status, container, path, (await r.text()).slice(0, 160));
