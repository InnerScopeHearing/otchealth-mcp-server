...
export function connectorToolset(env: Env, lane: string): Set<string> {
  ...
  // These fixed GitHub surfaces are CTO-only, so they must not inherit the shared ship set. Keep
  // the environment override semantics for CTO, but remove them from every non-CTO connector even
  // if a shared CONNECTOR_TOOLSET override accidentally names them.
  if (lane === 'cto') {
    if (!env.CONNECTOR_TOOLSET) {
      tools.add(CTO_ONLY_GITHUB_RECEIPT_TOOL);
      tools.add(CTO_ONLY_GITHUB_MAKE_BROKER_TOOL);
    }
  } else {
    tools.delete(CTO_ONLY_GITHUB_RECEIPT_TOOL);
    tools.delete(CTO_ONLY_GITHUB_MAKE_BROKER_TOOL);
  }
