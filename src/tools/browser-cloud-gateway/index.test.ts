...
test('registered session start derives owner from authenticated context', async () => {
  ...
  try { await requestContext.run({ callerHash: 'hash', correlationId: 'c', callerAgent: 'cto' }, () => handlers.get('browser_cloud_session_start')!({ profile_id: 'cto-profile', max_seconds: 60, dry_run: false })); assert.deepEqual(owners, ['cto']); } finally { process.env.CLOUD_BROWSER_ENABLED = oldEnabled; process.env.CLOUD_BROWSER_DDB_TABLE = oldTable; process.env.CLOUD_BROWSER_QUEUE_URL = oldQueue; process.env.CLOUD_BROWSER_ARTIFACT_BUCKET = oldBucket; process.env.READ_ONLY_MODE = oldReadOnly; process.env.ENABLE_WRITE_TOOLS = oldWrite; }
});
