...
import { driveConfigured, downloadFile, downloadFileHash } from '../../graph/drive-client.js';
 ...
 *
 * verify_sha256_only calls drive-client.ts's downloadFileHash, a DEDICATED function -- not
 * downloadFile()'s normal path with the bytes re-hashed afterward. Review finding, 2026-07-30
 * (round 2): the original hash-only implementation called downloadFile(..., forceBase64=true)
 * then did `Buffer.from(res.base64, 'base64')` to hash it, which meant THREE full-size copies of
 * the file in memory at once (the raw response buffer, the base64 string downloadFile builds, and
 * the re-decoded Buffer here) for a caller that only wants a 64-char hex digest. downloadFileHash
 * holds exactly one buffer for the call's lifetime -- see its own header for why this is not (yet)
 * a fully zero-buffer streaming hash.
 */
export async function handleGraphDriveDownload(input: GraphDriveDownloadInput, ctx: ToolContext): Promise<ToolResultPayload> {
  ...
  if (input.verify_sha256_only === true) {
    const res = await downloadFileHash(input.folder, input.filename);
    if (!res.found) {
      return { data: empty, summary: `No file "${input.filename}" in "${input.folder}".` };
    }
    return {
      data: { folder: input.folder, filename: input.filename, found: true, contentType: res.contentType, size: res.size, text: null, base64: null, sha256: res.sha256 },
      summary: `"${input.folder}/${input.filename}" is ${res.size} bytes, ${res.contentType ?? 'unknown type'}, sha256=${res.sha256}. Compare against the sha256 graph_drive_upload returned to confirm this file round-tripped intact.`,
    };
  }
