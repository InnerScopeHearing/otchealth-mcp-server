...
const edge = { edge_id: 'edge-1', predicate: 'scheduled_in', document_id: 'doc-1', source_id: 'a'.repeat(64), source_sha256: 'b'.repeat(64), text_sha256: 'c'.repeat(64), anchor_sha256: 'd'.repeat(64), locator_sha256: 'e'.repeat(64), status: 'reviewed', reviewer_sha256: 'f'.repeat(64) };
...
test('returns three separately proven exact pairs with provenance', async () => {
  ...
  assert.equal(result.data.x_y[0].source_id, 'a'.repeat(64));
  ...
  for (const call of h.calls) {
    ...
    assert.match(body.query, /entity_id: \$left_entity_id/);
  }
});
...
test('invalid matter and malformed provider provenance fail closed', async () => {
  const h = harness([[{ ...edge, source_id: 'bad' }], [], []]);
