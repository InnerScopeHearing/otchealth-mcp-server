 ...
 *
 * UNCONDITIONAL clo-personal BLOCK (2026-07-30 review): clo-personal IS an executive-ring member (so
 * it always passed the connector-lane check above), but it is a privilege-WALLED personal-legal lane
 * that must never write to memory_write's broadly-shared, brain_search-recallable memory-of-record --
 * this tool's own docstring has always claimed "clo-personal rejected," but nothing enforced it until
 * now. Fires regardless of connector surface (unlike the ship-lane check above, which only applies to
 * connector callers) -- a clo-personal client_credentials caller is refused too.
 */
export function memoryWriteRefusal(connectorSurface: boolean, lane: string): string | null {
  ...
  if (normalized === 'clo-personal') {
    return '"clo-personal" is a privilege-walled personal-legal lane and may never write to memory_write\'s broadly-shared, brain_search-recallable memory-of-record (this tool\'s own docstring has always said so; this is now code-enforced) -- use a room-scoped write path instead.';
  }
  if (connectorSurface && !isShipLane(normalized)) {
    return `Your connector lane ("${lane || '(none)'}") is not authorized to write fleet memory -- memory_write requires a ship-lane connector identity (cto/developer/executive-ring).`;
  }
  return null;
}

/**
 * SAFETY-CRITICAL IDENTITY-FORGERY GATE (2026-07-30 fix): memory_write is documented, and has always
 * been documented, as SELF-WRITE-ONLY -- unlike memory_remember (whose `agent` param genuinely picks
 * a cross-lane TARGET feed and separately, always, records the true writer into its own `by` field),
 * memory_write's `agent` input was the ONLY identity field on the record, and the handler passed it
 * straight through with ZERO check against the caller's authenticated token identity. That meant any
 * caller reachable to this tool could forge a byte-exact, broadly-recallable "system-of-record" entry
 * attributed to ANY OTHER lane -- e.g. a low-privilege caller writing
 ...
 * identity-forgery refusal path is directly testable through the ACTUAL registered entry point, not
 * only through the extracted pure gate functions (memoryWriteRefusal/memoryWriteIdentityRefusal).
 * Copilot review, 2026-07-30: "the new safety-critical tests exercise only the pure helper... Add a
 * handler-level regression that runs with an authenticated request context, submits a forged agent,
 * verifies written:false and no storage call... That is the actual exploit path this PR is intended
 * to close." See memory-write.test.ts's handler-level tests, run inside requestContext.run() (the
 * identity source here is currentCallerAgent(), an AsyncLocalStorage read -- NOT ctx.callerAgent --
