...
/** Drop corrections that a newer correction supersedes; keep newest-first order. Pure + testable. */
export function collapseSuperseded(entries: MemoryEntry[]): MemoryEntry[] {
  ...
  registerTool(
    server,
    {
      ...
      handler: async (input, ctx) => {
        ...
        if (!agentRaw) {
          return {
            data: { agent: '', pack: null, memory_records: [], tasks: null, inbox: null, inbound: null, errors: ['No agent specified and no caller identity; pass agent.'] },
        ...
        const packP = (async () => {
          ...
          const mine = (await readSharedAll()).filter((r) => r.agent === agent); // newest-first
        ...
        const [pack, memory, tasks, inbox, inbound] = await Promise.allSettled([packP, memP, tasksP, inboxP, inboundP]);
        ...
        return {
          data: { agent, pack: packV, memory_records: (memV as { records: unknown[] }).records, tasks: tasksV, inbox: inboxV, inbound: inboundV, errors },
