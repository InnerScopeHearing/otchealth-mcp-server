...
const CTO_ONLY_GITHUB_MAKE_BROKER_TOOL = 'github_make_broker';
...
test('(a) cto lane gets the full ship-lane set, including the privileged tools', () => {
  ...
  assert.deepEqual([...set].sort(), [...CTO_SHIP_LANE_TOOLSET, ...CLOUD_BROWSER_TOOLS, CTO_CLOUD_BROWSER_PROVISIONING_TOOL, 'hyperagent_discover_capabilities', CTO_ONLY_GITHUB_RECEIPT_TOOL, CTO_ONLY_GITHUB_MAKE_BROKER_TOOL].sort());
  ...
  assert.ok(set.has(CTO_ONLY_GITHUB_MAKE_BROKER_TOOL), 'CTO connector must expose the fixed GitHub Make pilot broker');
...
test('the fixed observation receipt and Make broker are visible only on the CTO connector, not shared ship lanes', () => {
  ...
  assert.equal(CTO_SHIP_LANE_TOOLSET.includes(CTO_ONLY_GITHUB_MAKE_BROKER_TOOL), false, 'CTO-only Make broker must not be in the shared ship set');
  assert.equal(connectorToolset(testEnv(), 'cto').has(CTO_ONLY_GITHUB_RECEIPT_TOOL), true);
  assert.equal(connectorToolset(testEnv(), 'cto').has(CTO_ONLY_GITHUB_MAKE_BROKER_TOOL), true);
  for (const lane of ['developer', 'exec', 'cfo', 'clo', 'clo-personal', 'cpo', 'cco', 'cro', 'coo', 'wefunder-campaign-director', 'external-read', 'unknown']) {
    ...
    assert.equal(connectorToolset(testEnv(), lane).has(CTO_ONLY_GITHUB_MAKE_BROKER_TOOL), false, `${lane} connector must not advertise the CTO-only Make broker`);
  }
  const overridden = { ...testEnv(), CONNECTOR_TOOLSET: `${CTO_ONLY_GITHUB_RECEIPT_TOOL},${CTO_ONLY_GITHUB_MAKE_BROKER_TOOL}` } as Env;
  ...
  assert.equal(connectorToolset(overridden, 'developer').has(CTO_ONLY_GITHUB_MAKE_BROKER_TOOL), false, 'a shared override must not leak the CTO-only Make broker');
});
