...
export function createS3BrowserArtifactStore(configInput: AwsBrowserJobsConfig = {}, deps: AwsBrowserJobsDeps = {}): CloudBrowserArtifactStore {
  ...
  return { async putImmutable(input) {
    ...
    const signed = signer({ method: 'PUT', host, path: `/${input.key}`, region: config.region, service: 's3', credentials: creds, body: Buffer.from(input.body), extraHeaders: { 'content-type': input.contentType, 'if-none-match': '*', 'x-amz-server-side-encryption': 'AES256', 'x-amz-meta-sha256': input.sha256, 'x-amz-content-sha256': actual } });
    ...
    const creds = await credentials(); if (!creds) noCredentials(); const host = `${config.bucket}.s3.${config.region}.amazonaws.com`; const signed = signer({ method: 'GET', host, path: `/${input.key}`, query: { versionId: input.version }, region: config.region, service: 's3', credentials: creds, extraHeaders: { 'x-amz-content-sha256': createHash('sha256').update('').digest('hex') } });
