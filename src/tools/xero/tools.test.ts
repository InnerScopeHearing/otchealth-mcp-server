...
// EXEC_RING-gated in-handler") and then lists them by name -- load-bearing documentation for
// anyone reasoning about the Xero surface, and documentation that silently rots the moment a tool
// is added or removed without touching it.
//
// This pins the inventory as a PURE LIST, deliberately: an earlier version of this test drove
// registerXeroTools against a stub McpServer, which runs registry.registerTool's entire stack
// (loadEnv, the capability catalog, per-lane catalog curation, connector-surface/M365 checks)
// outside any request context -- a lot of machinery to boot just to count names, and it failed in
// CI. Counting `registerTool(` occurrences in the source would be wrong by construction anyway,
// since three of these tools (contacts/payments/credit_notes) share ONE call site inside
// registerPagedAccountingRead. Adding a tool must update this list AND the header count.
// -------------------------------------------------------------------------------------------
...
test('xero tool inventory: exactly the 24 names tools.ts\'s module header claims, with no duplicates', () => {
  assert.equal(new Set(EXPECTED_XERO_TOOLS).size, EXPECTED_XERO_TOOLS.length, 'no name may appear twice');
  assert.equal(EXPECTED_XERO_TOOLS.length, 24, 'tools.ts\'s header says "24 tools" -- keep the two in step');
  assert.ok(EXPECTED_XERO_TOOLS.includes('xero_aggregate'), 'xero_aggregate (#291b) belongs in the inventory');
});

test('every xero_* tool is visible on the CTO ship lane -- the connector the CFO seat actually uses', async () => {
  // Sixth guard against this repo's documented omission class: xero_attachment_upload,
  // catalog_probe, kb_get_document, mail_archive_*, and xero_gl_assemble were each built,
  // registered and EXEC_RING-gated, yet invisible on every connector because nobody added them to
  // CTO_SHIP_LANE_TOOLSET. VISIBILITY only -- authorization stays the in-handler
  // isXeroAllowed(ctx.callerAgent) check, which is unaffected by this list.
  const { CTO_SHIP_LANE_TOOLSET } = await import('../registry.js');
  const shipLane = new Set<string>(CTO_SHIP_LANE_TOOLSET);
  const missing = EXPECTED_XERO_TOOLS.filter((name) => !shipLane.has(name));
  assert.deepEqual(missing, [], `these xero tools are registered but invisible on the CTO ship lane: ${missing.join(', ')}`);
});
