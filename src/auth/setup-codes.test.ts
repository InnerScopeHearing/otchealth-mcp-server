...
test('ELEVATION_ROLES is exactly the seven named roles, and clo-personal is not among them', () => {
  assert.deepEqual([...ELEVATION_ROLES].sort(), ['cfo', 'clo', 'coo', 'cro', 'cto', 'developer', 'wefunder-campaign-director'].sort());
  assert.equal((ELEVATION_ROLES as readonly string[]).includes('clo-personal'), false);
});

test('isElevationRole accepts exactly the seven roles and rejects everything else, including clo-personal', () => {
