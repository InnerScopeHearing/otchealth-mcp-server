...
export const GOVERNANCE: GovRule[] = [
  ...
  { pattern: 'github_pr_mark_ready_for_review', requiredRole: ['cto', 'developer'], reason: 'Changing a draft PR to ready for review is a write_simple GitHub action. The tool is narrowly bounded to that one transition and requires an expected head SHA, but it must remain limited to cto/developer.' },
