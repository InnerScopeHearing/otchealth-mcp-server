 ...
 *
 * The durable "poke the other agent now" channel that pairs with the Cosmos/Postgres work-ledger:
 ...
 *
 * One queue per agent: `inbox-<agent>`. agent_dispatch enqueues; inbox_read drains; wake peeks.
 * Inert without creds: if AGENT_INBOX_STORAGE_ACCOUNT/KEY are unset, isConfigured() is false.
 *
 * Selected by src/agentstate/queue.ts (the dispatcher) when STATE_BACKEND=cosmos. See
 * queue-postgres.ts for the STATE_BACKEND=postgres implementation of this exact surface.
 */
