...
import type {
  CompanyTextChunk,
  CompanyTextSnapshotResult,
  CompanyTextSource,
} from './company-text-snapshot.js';
...
type TextSource = CfoTextSource | CompanyTextSource;
type TextChunk = CfoTextChunk | CompanyTextChunk;
type TextSnapshotResult = CfoTextSnapshotResult | CompanyTextSnapshotResult;
type TextProfile = Readonly<{
  preparationSchema: string;
  chunkSchema: string;
  ...
  workerSeat: 'cfo' | 'clo';
  errorPrefix: 'cfo_text' | 'company_text';
}>;

const CFO_PROFILE: TextProfile = Object.freeze({
  preparationSchema: CFO_TEXT_PREPARATION_SCHEMA,
  chunkSchema: CFO_TEXT_CHUNK_RESPONSE_SCHEMA,
  snapshotSchema: CFO_TEXT_SNAPSHOT_SCHEMA,
  manifestSchema: 'cfo-text-prepared-manifest-v1',
  bundleSchema: 'cfo-text-chunk-bundle-v1',
  room: 'finance', sourceIndex: 'finance-cfo-source-docs', workerSeat: 'cfo',
  errorPrefix: 'cfo_text',
});
const COMPANY_PROFILE: TextProfile = Object.freeze({
  preparationSchema: 'company-text-preparation-v1',
  chunkSchema: 'company-text-prepared-chunk-v1',
  snapshotSchema: 'company-version-pinned-text-snapshot-v1',
  manifestSchema: 'company-text-prepared-manifest-v1',
  bundleSchema: 'company-text-chunk-bundle-v1',
  room: 'legal_company', sourceIndex: 'legal-company', workerSeat: 'clo',
  errorPrefix: 'company_text',
});
...
function active(signal: AbortSignal, profile: TextProfile): void {
  if (signal.aborted) fail(`${profile.errorPrefix}_preparation_deadline`);
}
function exact(value: unknown, keys: readonly string[]): value is Record<string, unknown> {
  return !!value && Object.getPrototypeOf(value) === Object.prototype &&
    Object.keys(value as Record<string, unknown>).sort().join('\0') === [...keys].sort().join('\0');
}
function boundedJson(response: StoreResponse, profile: TextProfile): unknown {
  if (response.body.length > MAX_STORE_RESPONSE_BYTES) fail(`${profile.errorPrefix}_preparation_corrupt`);
  try { return JSON.parse(response.body.toString('utf8')); }
  catch { fail(`${profile.errorPrefix}_preparation_corrupt`); }
}
...
function chunkMetadata(chunk: TextChunk) {
...
function validateChunk(chunk: TextChunk, expectedOrdinal: number, profile: TextProfile): void {
...
function validDescriptor(value: unknown, source: TextSource, chunkCount: number, profile: TextProfile): value is Record<string, unknown> {
  ...
  return value.schema === profile.snapshotSchema && value.room === profile.room &&
...
function buildBundles(runId: string, snapshotId: string, chunks: readonly TextChunk[], profile: TextProfile) {
  ...
  function flush() {
    ...
    const value = {
      schema: profile.bundleSchema, run_id: runId, snapshot_id: snapshotId,
      bundle_ordinal: ordinal, chunks: pending,
    };
    const body = canonical(value);
    if (Buffer.byteLength(body) > BUNDLE_MAX_BYTES) fail(`${profile.errorPrefix}_preparation_oversize`);
...
function createTextPreparationController(options: CfoTextPreparationControllerOptions, profile: TextProfile) {
  ...
  if (!Number.isSafeInteger(maxPreparedBytes) || maxPreparedBytes < 1 ||
    ...
    fail(`${profile.errorPrefix}_preparation_configuration`);
  }
  const prefix = `graph-trial/20260908/workers/${profile.workerSeat}/${options.runId}`;
  ...
  async function getExact(key: string, expectedBody: string, signal: AbortSignal): Promise<void> {
    ...
    if (response.status !== 200 || response.body.length !== Buffer.byteLength(expectedBody) ||
        response.body.toString('utf8') !== expectedBody) fail(`${profile.errorPrefix}_preparation_conflict`);
  }
  async function persistImmutable(key: string, body: string, signal: AbortSignal): Promise<void> {
    ...
    active(signal, profile);
    if (![200,201,409,412].includes(response.status)) fail(`${profile.errorPrefix}_preparation_unknown`);
    await getExact(key, body, signal);
    await recheck(signal);
  }
  ...
  return Object.freeze({
    async prepare(
      ...
      if (!exact(input, ['document_ordinal']) || !Number.isSafeInteger(input.document_ordinal) ||
          input.document_ordinal < 0 || input.document_ordinal >= 100) fail(`${profile.errorPrefix}_preparation_request`);
      await recheck(signal);
      ...
      if (snapshot.outcome !== 'ready') {
        return freeze({
          schema: profile.preparationSchema,
      ...
      snapshot.chunks.forEach((chunk, ordinal) => validateChunk(chunk, ordinal, profile));
      ...
      const bundles = buildBundles(options.runId, snapshotId, snapshot.chunks, profile);
      const bundleReferences: BundleReference[] = [];
      const chunkReferences: ChunkReference[] = [];
      for (const bundle of bundles) {
        const value = bundle.value;
        const bundleChunks = value.chunks as readonly TextChunk[];
    ...
    async readChunk(
      ...
      if (!exact(input, ['snapshot_id','ordinal']) || !SNAPSHOT_ID.test(input.snapshot_id || '') ||
        ...
        fail(`${profile.errorPrefix}_chunk_request`);
      }
      await recheck(signal);
      const manifestResponse = await options.store({ method: 'GET',
        key: manifestKey(prefix, input.snapshot_id), signal });
      active(signal, profile);
      if (manifestResponse.status === 404) fail(`${profile.errorPrefix}_chunk_not_found`);
      if (manifestResponse.status !== 200) fail(`${profile.errorPrefix}_chunk_unknown`);
      const manifestValue = boundedJson(manifestResponse, profile);
      if (!exact(manifestValue, ['schema','snapshot_id','identity','bundles','chunks','manifest_sha256']) ||
          manifestValue.schema !== profile.manifestSchema || manifestValue.snapshot_id !== input.snapshot_id ||
        ...
        fail(`${profile.errorPrefix}_preparation_corrupt`);
      }
      const identity = manifestValue.identity;
      if (!exact(identity, ['schema','run_id','document_ordinal','descriptor','chunks']) ||
          identity.schema !== profile.preparationSchema || identity.run_id !== options.runId ||
      ...
      active(signal, profile);
      if (!Object.isFrozen(source) || !validDescriptor(identity.descriptor, source, identity.chunks.length, profile)) {
        fail(`${profile.errorPrefix}_preparation_corrupt`);
      }
      await recheck(signal);
      ...
      if (!exact(chunkRef, ['ordinal','start_utf16','end_utf16','start_byte','end_byte','text_sha256','bundle_ordinal']) ||
          ...
          !Number.isSafeInteger(chunkRef.bundle_ordinal)) fail(`${profile.errorPrefix}_preparation_corrupt`);
      ...
      const bundle = boundedJson(bundleResponse, profile);
      if (!exact(bundle, ['schema','run_id','snapshot_id','bundle_ordinal','chunks']) ||
          bundle.schema !== profile.bundleSchema || bundle.run_id !== options.runId ||
          bundle.snapshot_id !== input.snapshot_id || bundle.bundle_ordinal !== bundleRef.ordinal ||
          !Array.isArray(bundle.chunks)) fail(`${profile.errorPrefix}_preparation_corrupt`);
      const chunk = bundle.chunks.find((candidate: unknown) =>
        ...
        TextChunk | undefined;
      if (!chunk) fail(`${profile.errorPrefix}_preparation_corrupt`);
      validateChunk(chunk, input.ordinal, profile);
      if (chunk.start_utf16 !== chunkRef.start_utf16 || chunk.end_utf16 !== chunkRef.end_utf16 ||
          ...
          chunk.text_sha256 !== chunkRef.text_sha256) fail(`${profile.errorPrefix}_preparation_corrupt`);
      await recheck(signal);
      return freeze({
        schema: profile.chunkSchema,
...
export interface CompanyTextPreparationControllerOptions extends Omit<CfoTextPreparationControllerOptions, 'sourceReader' | 'resolveSource'> {
  sourceReader: Readonly<{ readVersionPinnedPage: (source: CompanyTextSource, options: { signal: AbortSignal }) => Promise<CompanyTextSnapshotResult> }>;
  resolveSource: (documentOrdinal: number, options: { signal: AbortSignal }) => Promise<CompanyTextSource>;
}
