...
import { resolveCompanyGraphScope, type CompanyGraphScope, type CompanyGraphScopeId } from '../server/company-graph-scope.js';
import {
...
export const COMPANY_TEXT_SNAPSHOT_SCHEMA = 'company-version-pinned-text-snapshot-v1';
export const COMPANY_TEXT_MAX_SOURCE_BYTES = 8 * 1024 * 1024;
export const COMPANY_TEXT_MAX_CHUNK_CHARS = 16_000;
export const COMPANY_TEXT_MAX_CHUNK_BYTES = 16 * 1024;
export const COMPANY_TEXT_CHUNK_OVERLAP_CHARS = 200;
export const COMPANY_TEXT_DEADLINE_MS = 45_000;
...
export type CompanyTextSource = Readonly<{
...
export type CompanyTextChunk = Readonly<{
  ordinal: number;
  start_utf16: number;
  end_utf16: number;
  start_byte: number;
...
export type CompanyTextSnapshotResult =
  | Readonly<{
      outcome: 'ready';
      descriptor: Readonly<{
        schema: typeof COMPANY_TEXT_SNAPSHOT_SCHEMA;
        room: CompanyGraphScopeId;
        source_index: 'finance-cfo-source-docs' | 'legal-company';
      ...
      chunks: readonly CompanyTextChunk[];
    }>
  | Readonly<{
...
export interface CompanyTextSnapshotReaderOptions {
  /** Closed server-owned scope. Callers cannot supply storage locations or lanes. */
...
function active(signal: AbortSignal): void {
  if (signal.aborted) fail('company_text_deadline');
}
function abortable<T>(promise: Promise<T>, signal: AbortSignal): Promise<T> {
  active(signal);
  return new Promise<T>((resolve, reject) => {
    const abort = () => reject(Object.assign(new Error('company_text_deadline'), { code: 'company_text_deadline' }));
...
function validateSource(value: unknown, scope: CompanyGraphScope): CompanyTextSource {
  const keys = ['room','source_index','path','source_path_hash','document_version_id','source_version'];
  if (!Object.isFrozen(value) || !exact(value, keys)) fail('company_text_source_invalid');
  const source = value as unknown as CompanyTextSource;
  const path = normalizedSourcePath(source.path);
  if (!path || path !== source.path || source.room !== scope.room || source.source_index !== scope.sourceIndex ||
    ...
    fail('company_text_source_invalid');
  }
  return Object.freeze({ ...source });
}
function emptyResult(
  outcome: Exclude<CompanyTextSnapshotResult['outcome'], 'ready'>,
  source: CompanyTextSource,
...
function chunkText(text: string): readonly CompanyTextChunk[] {
  const chunks: CompanyTextChunk[] = [];
  const byteOffsets = utf8Offsets(text);
  let start = 0;
  while (start < text.length) {
    let low = start + 1;
    let high = Math.min(text.length, start + COMPANY_TEXT_MAX_CHUNK_CHARS);
    let end = start;
    while (low <= high) {
      const candidate = Math.floor((low + high) / 2);
      if (byteOffsets[candidate] - byteOffsets[start] <= COMPANY_TEXT_MAX_CHUNK_BYTES) {
        end = candidate;
        low = candidate + 1;
      } else {
        high = candidate - 1;
    ...
    if (end <= start) fail('company_text_chunk_invalid');
    ...
    let next = Math.max(start + 1, end - COMPANY_TEXT_CHUNK_OVERLAP_CHARS);
...
/**
 * Internal company-scope source reader. The server must construct this with the trusted
 ...
 */
export function createCompanyTextSnapshotReader(options: CompanyTextSnapshotReaderOptions) {
  const resolved = resolveCompanyGraphScope(options?.callerContext?.caller_agent, options?.scope?.scope);
  if (!options || options.callerContext?.connector_surface !== true || !resolved.ok ||
      resolved.scope !== options.scope || !isLaneAllowed(options.scope.sourceIndex, options.callerContext.caller_agent)) fail('company_text_forbidden');
  const location = options.scope.sourceStore;
  const fixedLocation = Object.freeze({ ...location });
  const region = options.region ?? 'us-east-1';
  const deadlineMs = options.deadlineMs ?? COMPANY_TEXT_DEADLINE_MS;
  const maxSourceBytes = options.maxSourceBytes ?? COMPANY_TEXT_MAX_SOURCE_BYTES;
  if (!/^[a-z]{2}(?:-gov)?-[a-z]+-\d$/.test(region) ||
      !Number.isSafeInteger(deadlineMs) || deadlineMs < 1000 || deadlineMs > COMPANY_TEXT_DEADLINE_MS ||
      !Number.isSafeInteger(maxSourceBytes) || maxSourceBytes < 1 || maxSourceBytes > COMPANY_TEXT_MAX_SOURCE_BYTES ||
    ...
    fail('company_text_configuration');
  }
  ...
  async function request(
    ...
    const rawPath = `/${fixedLocation.prefix}${key}`;
  ...
  return Object.freeze({
    async readVersionPinnedPage(sourceValue: CompanyTextSource, { signal }: { signal?: AbortSignal } = {}): Promise<CompanyTextSnapshotResult> {
      const source = validateSource(sourceValue, options.scope);
      ...
      active(boundedSignal);
      if (!credentials) fail('company_text_credentials_unavailable');
      ...
      if (head.status !== 200) fail('company_text_source_unavailable');
      ...
      if (!etag || !ETAG.test(etag) || !boundedVersion(versionId) || length === null) {
        fail('company_text_source_unavailable');
      }
      ...
      const descriptor = Object.freeze({
        schema: COMPANY_TEXT_SNAPSHOT_SCHEMA,
        room: options.scope.room,
        source_index: options.scope.sourceIndex,
        source_document_version: source.document_version_id,
        catalog_source_sha256: source.source_version,
        ...
        chunk_overlap_chars: COMPANY_TEXT_CHUNK_OVERLAP_CHARS,
      });
