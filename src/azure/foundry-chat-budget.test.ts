...
// Chat calls now carry an explicit, env-bounded timeout; embeddings keep the default, and the
// retry count is left at fetchWithBudget's default for both (see flex-429-default-fallback.test.ts).

test('chatRequestBudget: defaults to 90 s with no retry when the env var is unset', () => {
  assert.deepEqual(chatRequestBudget({}), { timeoutMs: 90_000 });
  assert.deepEqual(chatRequestBudget({ FOUNDRY_CHAT_TIMEOUT_MS: undefined }), { timeoutMs: 90_000 });
});

test('chatRequestBudget: honors an in-range FOUNDRY_CHAT_TIMEOUT_MS', () => {
  assert.deepEqual(chatRequestBudget({ FOUNDRY_CHAT_TIMEOUT_MS: 120_000 }), { timeoutMs: 120_000 });
  assert.deepEqual(chatRequestBudget({ FOUNDRY_CHAT_TIMEOUT_MS: 8_000 }), { timeoutMs: 8_000 });
  assert.deepEqual(chatRequestBudget({ FOUNDRY_CHAT_TIMEOUT_MS: 300_000 }), { timeoutMs: 300_000 });
});
...
test('chatRequestBudget: never overrides the retry count (flex-fallback attempt accounting depends on the default)', () => {
  assert.equal('retries' in chatRequestBudget({}), false);
});
