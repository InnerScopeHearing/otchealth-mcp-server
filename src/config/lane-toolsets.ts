 ...
 * (2026-08-02) to be a real, currently-registered canonical tool name in this repo. ONLY affects
 * what curate-m365-only advertises to M365 static-token requests on the cto/cro lanes -- 'exec'
 * keeps the full CTO_INFRA wildcard breadth (it has no M365 static token and is never curated under
 * the live mode), and a Claude Code / Hyperagent client_credentials session on cto/cro is NEVER
 * curated by this table either way (see tool-catalog-curation.ts's isM365 gate). To refresh: rerun
 * build-agents.mjs, extract each file's `.map(t => t.name)`, and replace the array body below.
 */
