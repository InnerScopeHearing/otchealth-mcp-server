...
async function composition(config,env,stdout){
 if(!exact(config,['schema','cto_root','host_config','outbox_directory','cfo_project_config','producer','registry','review_mode','review_model'])||config.schema!=='relationship-backfill-runtime-v1'||!['cto_root','host_config','outbox_directory','cfo_project_config'].every(k=>typeof config[k]==='string'&&isAbsolute(config[k]))||!/^[a-z][a-z0-9-]{0,63}$/.test(config.producer??'')||!['gpt-5.6-luna','gpt-5.6-sol','gpt-6-astra'].includes(config.review_model))fail('configuration');
 ...
 const cfoBearer=createCfoProjectBearerTokenProvider({configPath:config.cfo_project_config});
 ...
 await cfoBearer();
 const bearerTokenProvider=cfoBearer;
 const getAuthorization=async(request,options)=>`Bearer ${await bearerTokenProvider(request,options)}`;
 const client=candidateOnly?null:partitioned?registryClient.createGatewayPartitionIdentityRegistryClient({registryId:r.id,manifestVersion:r.version,authority:r.authority,callerSeat:'cfo',bearerTokenProvider}):registryClient.createGatewayIdentityRegistryClient({registryId:r.id,authority:r.authority,callerSeat:'cfo',bearerTokenProvider});
...
export async function runFullBackfillCli({argv=process.argv.slice(2),env=process.env,stdout=process.stdout,stderr=process.stderr}={}){
 try{
  ...
  if(args.mode==='check'){stdout.write(JSON.stringify({schema:'relationship-launcher-status-v1',status:'configured',review_mode:config.review_mode,review_model:config.review_model,processing:'relationships',live_verified:false})+'\n');return 0;}
  ...
  stdout.write(JSON.stringify({schema:'relationship-launcher-status-v1',status:'starting',review_mode:config.review_mode,review_model:config.review_model,processing:'relationships',live_verified:false})+'\n');
