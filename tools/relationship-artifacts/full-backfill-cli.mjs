...
async function composition(config,env,stdout){
 if(!exact(config,['schema','cto_root','host_config','outbox_directory','cfo_project_config','producer','registry','review_mode'])||config.schema!=='relationship-backfill-runtime-v1'||!['cto_root','host_config','outbox_directory','cfo_project_config'].every(k=>typeof config[k]==='string'&&isAbsolute(config[k]))||!/^[a-z][a-z0-9-]{0,63}$/.test(config.producer??''))fail('configuration');
