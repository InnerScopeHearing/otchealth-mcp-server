...
@{schema='relationship-backfill-runtime-v1';cto_root=$ctoDirectory;host_config=$hostFile;outbox_directory=(Join-Path $runtimeRoot 'outbox');producer=$Producer;review_mode=$ReviewMode;registry=$registryConfig} | ConvertTo-Json -Depth 8 | Write-Utf8File -Path (Join-Path $runtimeRoot 'runtime.json')
...
Write-Output 'Prepared only. Scheduler not registered or activated. CFO token is read from the existing environment at runtime.'
