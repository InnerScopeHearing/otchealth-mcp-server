...
@{schema='relationship-backfill-runtime-v1';cto_root=$ctoDirectory;host_config=$hostFile;outbox_directory=(Join-Path $runtimeRoot 'outbox');cfo_project_config=$cfoConfig;producer=$Producer;review_mode=$ReviewMode;review_model=$ReviewModel;registry=$registryConfig} | ConvertTo-Json -Depth 8 | Write-Utf8File -Path (Join-Path $runtimeRoot 'runtime.json')
