...
export function createReviewHistoryAuthority({run,producer,callerSeat='cfo',getAuthorization,fetchImpl=globalThis.fetch}){
 ...
 async function requestDecision(request,{signal}={}){
  ...
  const p=value?.provenance;if(value.allowed!==true||value.authorization_request_sha256!==hash(request)||p?.decision_source!=='authenticated_resolution_store'||p.authenticated_store_id!==HISTORY_STORE_ID||p.authenticated_producer_id!==producer||typeof p.policy_version!=='string'||!p.policy_version||!Array.isArray(p.allowed_roles)||!p.allowed_roles.includes(callerSeat))fail('relationship_authority_invalid');return value;
 }
 return Object.freeze({authorizeHistory:async(request,options)=>{if(canonical(request.run)!==canonical(run)||request.caller_seat!==callerSeat||request.store_id!==HISTORY_STORE_ID||!['read','write'].includes(request.action))fail('relationship_authority_scope');return requestDecision(request,options);},authorizeArtifact:async request=>{
  if(canonical(request.scope)!==canonical({run,caller_seat:callerSeat,producer_id:producer})||!['put','get'].includes(request.action))fail('relationship_authority_scope');
