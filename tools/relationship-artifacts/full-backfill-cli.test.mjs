...
test('ordinary executable loads real relationship factory and registry client without caller injection',{skip:!process.env.RELATIONSHIP_CTO_ROOT},async()=>{
 ...
 const check=execute('--check');assert.equal(check.status,0,check.stderr);assert.match(check.stdout,/configured/);assert.match(check.stdout,/"review_model":"gpt-5.6-sol"/);assert.match(check.stdout,/"extractor_model":"gpt-5.6-terra"/);assert.match(check.stdout,/"live_verified":false/);
