...
test('ordinary executable loads real relationship factory and registry client without caller injection',{skip:!process.env.RELATIONSHIP_CTO_ROOT},async()=>{
 ...
 try{
 const {publicKey,privateKey}=generateKeyPairSync('ed25519'),keyFile=join(root,'public.pem'),hostFile=join(root,'host.json'),configFile=join(root,'runtime.json'),preload=join(root,'transport.mjs'),projectConfig=join(root,'CFO','.codex','config.toml');
 const binary=join(root,'codex.exe');await writeFile(binary,'synthetic never executed');await (await import('node:fs/promises')).mkdir(join(root,'CFO','.codex'),{recursive:true});await writeFile(projectConfig,'[mcp_servers.otchealth.http_headers]\nAuthorization = "Bearer synthetic-test-token-not-real"\n');await writeFile(keyFile,publicKey.export({type:'spki',format:'pem'}));await writeFile(hostFile,JSON.stringify({schema:'company-catalog-controller-host-v1',seat:'cfo',binary,cohort_id:'synthetic'}));
 ...
 await writeFile(configFile,JSON.stringify({schema:'relationship-backfill-runtime-v1',review_mode:'signed-review',review_model:'gpt-5.6-terra',extractor_model:'gpt-5.6-terra',cto_root:resolve(process.env.RELATIONSHIP_CTO_ROOT),host_config:hostFile,outbox_directory:join(root,'outbox'),cfo_project_config:projectConfig,producer:'reviewer',registry:{id:'synthetic',version:'1',public_key_file:keyFile,authority:{schema:'authenticated-structured-identity-authority-v1',scope:'cfo',adapter_id:'synthetic',source_system:'synthetic',version:'1'}}}));
 ...
 const execute=mode=>spawnSync(process.execPath,['--import',pathToFileURL(preload).href,cli,mode,'--config',configFile],{encoding:'utf8',windowsHide:true,timeout:30000,env:{...process.env,CODEX_CFO_MCP_TOKEN:undefined}});
 const check=execute('--check');assert.equal(check.status,0,check.stderr);assert.match(check.stdout,/configured/);assert.match(check.stdout,/"review_model":"gpt-5.6-terra"/);assert.match(check.stdout,/"extractor_model":"gpt-5.6-terra"/);assert.match(check.stdout,/"live_verified":false/);
 const unsupported={...JSON.parse(await (await import('node:fs/promises')).readFile(configFile,'utf8')),review_model:'gpt-5.6-sol'};await writeFile(configFile,JSON.stringify(unsupported));const rejected=execute('--check');assert.equal(rejected.status,2);assert.match(rejected.stderr,/not-ready/);await writeFile(configFile,JSON.stringify({...unsupported,review_model:'gpt-5.6-terra'}));
