...
export function createRelationshipPublicationPipeline({cohort_id,producer_id,outbox,publisher,loadExtracted,createReview,now=Date.now,onMonitor=()=>{}}={}){
 ...
 async function recover({signal,limit=10,after=recoveryCursor,skipIntentOnly=false}={}){
  ...
  recoveryCursor=page.next_after;return{status:events.find(x=>x.status!=='complete')?.status??(page.next_after?'dispatching':'complete'),code:events.some(x=>x.status!=='complete')?'relationship_recovery_held':page.next_after?'relationship_recovery_page':'relationship_recovery_complete',pending:page.items.length,events,next_after:page.next_after,monitor};
 }
