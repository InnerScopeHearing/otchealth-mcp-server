...
For the read-only live check, use the existing CFO project credential provider and supply the source binding hash. The hash must be the canonical SHA-256 binding identifier supplied by the signed source coverage export. The command calls only `https://mcp.otchealth.app`, rejects caller-supplied origins, bounds the response to 16 KiB, and accepts readiness only when the deployed gateway returns both `coverage_ready: true` and `reason: "coverage_checked"`. An environment token is an optional fallback only when no CFO project configuration is provided.

```powershell
node tools/identity-registry-production-preflight.mjs --config C:\secure\identity-registry-config.json --check-live --cfo-project-config C:\path\to\CFO\.codex\config.toml --source-binding-sha256 <64-lowercase-hex>
```
